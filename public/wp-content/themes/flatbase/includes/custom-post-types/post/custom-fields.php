 ...
 *
 * This file contains the function to hook custom fields for posts.
 *
 * @package   Flatbase
...
if ( ! function_exists( 'nice_custom_fields_post' ) ) :
add_filter( 'nice_custom_fields', 'nice_custom_fields_post', 10, 2 );
/**
 ...
 * @return array
 */
function nice_custom_fields_post( array $nice_fields = array(), $post_type ) {
	if ( 'post' === $post_type ) {
		/**
		 * Header
		 */
		...
		$nice_fields[] = array(
			...
			'options'   => array(
				''           => esc_html__( 'Inherit', 'nicethemes' ),
				'default'    => esc_html__( 'Default CSS', 'nicethemes' ),
			...
			'condition' => '_post_heading:is(true)',
			'operator'  => 'and',
		);
		...
		if ( ! nice_bool( nice_get_option( 'nice_post_author' ) ) ) {
			$nice_fields[] = array(
				'name'      => '_post_author_display',
				'std'       => 'false',
				'label'     => esc_html__( 'Display Post Author', 'nicethemes' ),
				'type'      => 'checkbox',
				'desc'      => sprintf( esc_html__( 'Display the post author for this post (it\'s set in %1$sTheme Settings%2$s to be hidden by default).', 'nicethemes' ), sprintf( '<a href="%s">', nice_admin_page_get_link( 'options' ) ), '</a>' ),
			);
		}

		if ( nice_bool( nice_get_option( 'nice_post_author' ) ) ) {
			$nice_fields[] = array(
				'name'      => '_post_author_hide',
				'std'       => 'false',
				'label'     => esc_html__( 'Hide Post Author', 'nicethemes' ),
				'type'      => 'checkbox',
				'desc'      => sprintf( esc_html__( 'Hide the post author for this post (it\'s set in %1$sTheme Settings%2$s to be displayed by default).', 'nicethemes' ), sprintf( '<a href="%s">', nice_admin_page_get_link( 'options' ) ), '</a>' ),
			);
		}
