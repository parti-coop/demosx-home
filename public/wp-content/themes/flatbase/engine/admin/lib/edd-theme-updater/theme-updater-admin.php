...
class EDD_Theme_Updater_Admin {
	...
	/**
	 * Obtain status message for the current license.
	 *
	 * @since  2.0.6
	 *
	 * @return string
	...
	function get_license_message() {
		if ( ! $this->system_status->get_nice_license_key() ) {
			return $this->get_string( 'enter-key' );
		}

		if ( ! empty( $_GET['settings-updated'] ) ) {
			delete_transient( $this->get_theme_slug() . '_license_message' );
		}

		$message = get_transient( $this->get_theme_slug() . '_license_message' );

		if ( ! $message ) {
			set_transient( $this->get_theme_slug() . '_license_message', $this->check_license(), ( 60 * 60 * 24 ) );
			$message = get_transient( $this->get_theme_slug() . '_license_message' );
		}
	...
	function check_license() {
		...
		if ( ! get_option( $this->theme_slug . '_license_key_status' ) ) {
